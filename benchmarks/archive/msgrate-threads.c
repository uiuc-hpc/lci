...
void* sendmsg(void* input)
{
  ...
  for (niters = 0; niters < t_info->iters; niters++) {
    MPI_Isend(send_buf + (nbytes * niters), nbytes, MPI_CHAR,
              t_info->rank + (world_size / 2), magic_tag, MPI_COMM_WORLD,
...
void* recvmsg(void* input)
{
  ...
  for (niters = 0; niters < t_info->iters; niters++) {
    MPI_Irecv(send_buf + (nbytes * niters), nbytes, MPI_CHAR,
              t_info->rank - (world_size / 2), magic_tag, MPI_COMM_WORLD,
              &t_info->local_reqs[nreqs++]);
  }
  MPI_Waitall(nreqs, t_info->local_reqs, MPI_STATUSES_IGNORE);
...
void test_one_way(void)
{
  ...
  if (world_size % 2 == 1) {
    MPI_Comm_split(MPI_COMM_WORLD, (rank == world_size - 1) ? MPI_UNDEFINED : 1,
                   rank, &comm);
  } else {
    ...
    if (rank < world_size / 2) {
      for (i = 0; i < niters; ++i) {
        ...
        for (thread_count = 0; thread_count < threads; thread_count++) {
          ...
          pthread_create(&pthreads[thread_count], NULL, sendmsg,
      ...
      for (i = 0; i < niters; ++i) {
        ...
        for (thread_count = 0; thread_count < threads; thread_count++) {
          ...
          info[thread_count].local_reqs =
              &reqs[(nmsgs / threads) * thread_count];
          info[thread_count].rank = rank;
          pthread_create(&pthreads[thread_count], NULL, recvmsg,
                         (void*)&info[thread_count]);
        }
        for (thread_count = 0; thread_count < threads; thread_count++) {
          pthread_join(pthreads[thread_count], NULL);
...
void* same_dir_run(void* input)
{
  ...
  for (j = 0; j < npeers; ++j) {
    nreqs = 0;
    for (niters = 0; niters < t_info->iters; ++niters) {
      MPI_Irecv(recv_buf + (nbytes * (niters + j * nmsgs)), nbytes, MPI_CHAR,
                recv_peers[j], magic_tag, MPI_COMM_WORLD,
                &t_info->local_reqs[nreqs++]);
    }
    for (niters = 0; niters < t_info->iters; ++niters) {
      MPI_Isend(send_buf + (nbytes * (niters + j * nmsgs)), nbytes, MPI_CHAR,
...
void test_same_dir(void)
{
  ...
  for (i = 0; i < niters; ++i) {
    ...
    for (thread_count = 0; thread_count < threads; thread_count++) {
      ...
      info[thread_count].rank = rank;
      pthread_create(&pthreads[thread_count], NULL, same_dir_run,
                     (void*)&info[thread_count]);
    }
    for (thread_count = 0; thread_count < threads; thread_count++) {
      pthread_join(pthreads[thread_count], NULL);
...
void test_same_direction(void)
{
  ...
  for (i = 0; i < niters; ++i) {
    ...
    for (j = 0; j < npeers; ++j) {
      nreqs = 0;
      for (k = 0; k < nmsgs; ++k) {
        MPI_Irecv(recv_buf + (nbytes * (k + j * nmsgs)), nbytes, MPI_CHAR,
                  recv_peers[j], magic_tag, MPI_COMM_WORLD, &reqs[nreqs++]);
      }
      for (k = 0; k < nmsgs; ++k) {
        MPI_Isend(send_buf + (nbytes * (k + j * nmsgs)), nbytes, MPI_CHAR,
                  send_peers[npeers - j - 1], magic_tag, MPI_COMM_WORLD,
                  &reqs[nreqs++]);
      }
...
int main(int argc, char* argv[])
{
  ...
  if (prov != MPI_THREAD_MULTIPLE) abort();
