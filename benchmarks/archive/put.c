...
int main(int argc, char** args)
{
  ...
  if (rank == 0) {
    ...
    while (!lc_test(&s)) lc_progress(mv);

    int size, rank, tag;
    lc_ctx r;
    while (!lc_recv_queue(mv, &size, &rank, &tag, &r)) lc_progress(mv);
    buf2 = malloc(size);
  ...
  for (int size = MIN_MSG_SIZE; size <= MAX_MSG_SIZE; size <<= 1) {
    ...
    for (int i = 0; i < TOTAL + SKIP; i++) {
      if (rank == 0) {
        ...
        while (!lc_test(&c2)) lc_progress(mv);

        while (!lc_test(&c1)) lc_progress(mv);

        if (i == 0)
          for (int j = 0; j < size; j++) assert(buf[j] == 'A');
      } else {
        while (!lc_test(&c2)) lc_progress(mv);
        if (i == 0)
          for (int j = 0; j < size; j++) assert(buf[j] == 'A');
