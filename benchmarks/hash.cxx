...
void benchmark_insert_with_delete(hash_init_f init, hash_insert_f insert)
{
  ...
  for (int j = 0; j < TOTAL_LARGE; j++) {
    ...
    for (int tt = 0; tt < NTHREADS; tt++) {
      th[tt] = std::move(
          std::thread([tt, j, &insert, &times, &my_table, &ti2, &f1, &f2] {
  ...
  for (auto& q : qu)
    q = q * 1e6 / (NUM_INSERTED_PER_THREAD);  // / (TOTAL_LARGE - SKIP_LARGE);
...
int main(int argc, char** args)
{
  ...
  benchmark_insert_with_delete<arr_hash_val, SERVER>(arr_hash_init,
                                                     arr_hash_insert);
  ...
  benchmark_insert_with_delete<arr_hash_val, THREADS>(arr_hash_init,
                                                      arr_hash_insert);
