...
void recvR(intptr_t) {
    ...
    for(int i=1; i<blockDimX+1; i++) {
        new_temperature[i][j] =
            (temperature[i-1][j]+temperature[i+1][j]+temperature[i][j-1]+temperature[i][j+1]+temperature[i][j]) * 0.2;
    }
...
void recvB(intptr_t) {
    ...
    for(int j=1; j<blockDimY+1; j++) {
      int i2 = blockDimX;
      new_temperature[i2][j] = (temperature[i2-1][j]+temperature[i2+1][j]+temperature[i2][j-1]+temperature[i2][j+1]+temperature[i2][j]) * 0.2;
    }
}
...
void recvT(intptr_t) {
    ...
    for(int j=1; j<blockDimY+1; j++) {
      int i1 = 1;
      new_temperature[i1][j] = (temperature[i1-1][j]+temperature[i1+1][j]+temperature[i1][j-1]+temperature[i1][j+1]+temperature[i1][j]) * 0.2;
    }
}
