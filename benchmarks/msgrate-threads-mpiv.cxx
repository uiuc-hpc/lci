...
void sendmsg(intptr_t input)
{
  ...
  {
    MPIV_Isend(send_buf + (nbytes * niters),
        nbytes,
        MPI_CHAR,
        t_info->rank + (world_size / 2),
        t_info->thread_id * t_info->iters + niters,
...
void recvmsg(intptr_t input)
{
  ...
  thread_input *t_info = (thread_input *)input;
  for (niters = 0; niters < t_info->iters; niters++){
    MPIV_Irecv(send_buf + (nbytes * niters),
        nbytes, MPI_CHAR, t_info->rank - (world_size / 2), t_info->thread_id * t_info->iters + niters,
        MPI_COMM_WORLD, &t_info->local_reqs[nreqs++]);
  }
  MPIV_Waitall(nreqs, t_info->local_reqs, MPI_STATUSES_IGNORE);
}
...
test_one_way(void)
{
  ...
  reqs = (MPIV_Request*) malloc(sizeof(MPIV_Request) * 2 * nmsgs * npeers * threads);
    ...
    if (rank < world_size / 2) {
      for (i = 0 ; i < niters ; ++i) {
        ...
        for (thread_count = 0; thread_count < threads; thread_count++){
          ...
          thread[thread_count] = MPIV_spawn(thread_count, sendmsg, (intptr_t) &info[thread_count]);
        }
        for (thread_count = 0; thread_count < threads; thread_count++){
      ...
      for (i = 0 ; i < niters ; ++i) {
        ...
        for (thread_count = 0; thread_count < threads; thread_count++){
          ...
          info[thread_count].local_reqs = &reqs[(nmsgs/threads)*thread_count];
          info[thread_count].rank = rank;
          thread[thread_count] = MPIV_spawn(thread_count, recvmsg, (intptr_t) &info[thread_count]);                    
        }
        for (thread_count = 0; thread_count < threads; thread_count++){
          MPIV_join(thread[thread_count]);
        } 
...
void same_dir_run(intptr_t input)
{
  ...
  for (j = 0 ; j < npeers ; ++j) {
    nreqs = 0;
    for (niters = 0 ; niters < t_info->iters ; ++niters) {
      MPIV_Irecv(recv_buf + (nbytes * (niters + j * nmsgs)),
          nbytes, MPI_CHAR, recv_peers[j], niters + t_info->thread_id * t_info->iters,
          MPI_COMM_WORLD,  &t_info->local_reqs[nreqs++]);
    }
    for (niters = 0 ; niters < t_info->iters ; ++niters) {
      MPIV_Isend(send_buf + (nbytes * (niters + j * nmsgs)),
          nbytes, MPI_CHAR, send_peers[npeers - j - 1], niters + t_info->thread_id * t_info->iters,
          MPI_COMM_WORLD,  &t_info->local_reqs[nreqs++]);
    }
    MPIV_Waitall(nreqs, t_info->local_reqs, MPI_STATUSES_IGNORE);
  }
...
test_same_dir(void)
{
  ...
  for (i = 0 ; i < niters ; ++i) {
    ...
    for (thread_count = 0; thread_count < threads; thread_count++){
      ...
      info[thread_count].rank = rank;
      thread[thread_count] = MPIV_spawn(thread_count, same_dir_run, (intptr_t)&info[thread_count]); 
    }
    for (thread_count = 0; thread_count < threads; thread_count++){
      MPIV_join(thread[thread_count]);
    } 
...
test_same_direction(void)
{
  ...
  for (i = 0 ; i < niters ; ++i) {
    ...
    for (j = 0 ; j < npeers ; ++j) {
      nreqs = 0;
      for (k = 0 ; k < nmsgs ; ++k) {
        MPIV_Irecv(recv_buf + (nbytes * (k + j * nmsgs)),
            nbytes, MPI_CHAR, recv_peers[j], magic_tag, 
            MPI_COMM_WORLD, &reqs[nreqs++]);
      }
      for (k = 0 ; k < nmsgs ; ++k) {
        MPIV_Isend(send_buf + (nbytes * (k + j * nmsgs)),
