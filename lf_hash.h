...
#define REVERSE_BYTE(x) ((so_lf_key_t)((((((uint32_t)(x)) * 0x0802LU & 0x22110LU) | (((uint32_t)(x)) * 0x8020LU & 0x88440LU)) * 0x10101LU >> 16) & 0xff))
#if 0
/* 32-bit reverse */
# define REVERSE(x) (REVERSE_BYTE((((so_lf_key_t)(x))) & 0xff) << 24) | \
    (REVERSE_BYTE((((so_lf_key_t)(x)) >> 8) & 0xff) << 16) |            \
    (REVERSE_BYTE((((so_lf_key_t)(x)) >> 16) & 0xff) << 8) |            \
    (REVERSE_BYTE((((so_lf_key_t)(x)) >> 24) & 0xff))
#else
# define REVERSE(x) ((REVERSE_BYTE((((so_lf_key_t)(x))) & 0xff) << 56) |       \
...
static inline so_lf_key_t so_dummykey(const key_t key)
{
    return REVERSE(key);
}
...
qt_hash qt_hash_create(int needSync)
{
    ...
    tmp->count = 0;
    {
        hash_entry *dummy = (hash_entry*) calloc(1, sizeof(hash_entry)); // XXX: should pull out of a memory pool
        assert(dummy);
