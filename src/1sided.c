...
{ 
  ...
  lc_server_put(ep->server, rep->handle, rep->base, offset, rep->rkey, size, MAKE_PROTO(ep->gid, LC_PROTO_LONG, meta), p);
  return LCI_OK;
}
...
{ 
  ...
  p->context.sync = sync;
  struct lc_rep* rep = &(ep->rep[rank]);
  lc_server_send(ep->server, rep->handle, size, p, MAKE_PROTO(ep->gid, LC_PROTO_LONG, meta));
  return LCI_OK;
}
