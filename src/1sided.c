...
LCI_error_t LCI_putbc(void* src, size_t size, int rank, int rma_id, int offset, LCI_endpoint_t ep)  
{ 
  ...
  lc_server_putm(ep->server, rep->handle, rep->base, offset, rep->rkey, size, p);
  return LCI_OK;
}

LCI_error_t LCI_putb(LCI_bdata_t buffer, size_t size, int rank, int rma_id, int offset, LCI_endpoint_t ep, void* sync)  
{ 
