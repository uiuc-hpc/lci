...
LCI_error_t LCI_putva(LCI_endpoint_t ep, LCI_iovec_t iovec,
  ...
  for (int i = 0; i < iovec.count; ++i) {
    LCM_DBG_Assert((iovec.lbuffers[0].segment == LCI_SEGMENT_ALL &&
  ...
  if (packet == NULL)
    // no packet is available
    return LCI_ERR_RETRY;
  packet->context.poolid = (iovec.piggy_back.length > LCI_PACKET_RETURN_THRESHOLD) ?
                            lc_pool_get_local(ep->pkpool) : -1;
  ...
  LCM_DBG_Log(LCM_LOG_DEBUG, "rdv", "send rts: type %d sctx %p count %d "
