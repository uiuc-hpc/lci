...
ofi_net_context_impl_t::ofi_net_context_impl_t(runtime_t runtime_, net_context_t::config_t config_) 
    ...
    FI_SAFECALL(fi_getinfo(FI_VERSION(1, 6), nullptr, nullptr, 0, hints, &all_infos));
    ...
    LCIXX_Log(LOG_INFO, "ofi", "Control progress mode: %s\n",
            fi_tostr(&(ofi_info->domain_attr->control_progress),
                    FI_TYPE_PROGRESS));
    LCIXX_Log(
...
: net_device_impl_t(context_, config_) {
  ...
  FI_SAFECALL(fi_endpoint(ofi_domain, p_ofi_context->ofi_info,
                          &ofi_ep, nullptr));
  ...
  // Bind my ep to cq.
  FI_SAFECALL(
      fi_ep_bind(ofi_ep, (fid_t)ofi_cq, FI_TRANSMIT | FI_RECV));
  ...
  for (int i = 0; i < runtime.get_nranks(); i++) {
    if (i != runtime.get_rank()) {
      ...
      int ret = fi_av_insert(ofi_av, (void*)peer_addr, 1,
                             &peer_addrs[i], 0, nullptr);
      LCIXX_Assert(ret == 1, "fi_av_insert failed! ret = %d\n", ret);
    } else {
      int ret = fi_av_insert(ofi_av, (void*)my_addr, 1,
                             &peer_addrs[i], 0, nullptr);
      LCIXX_Assert(ret == 1, "fi_av_insert failed! ret = %d\n", ret);
    }
  }
