...
class ofi_net_device_impl_t : public lcixx::net_device_impl_t
{
 public:
  ...
  std::vector<net_endpoint_t> net_endpoints;
  int next_to_poll;
};
...
{
 public:
  std::vector<struct fid_mr*> ofi_mrs;
};

inline rkey_t ofi_net_device_impl_t::get_rkey(mr_t mr)
{
  ofi_mr_impl_t& p_mr = *static_cast<ofi_mr_impl_t*>(mr.p_impl);
  rkey_t ret(p_mr.ofi_mrs.size() * sizeof(uint64_t));
  for (int i = 0; i < p_mr.ofi_mrs.size(); i++) {
    auto& ofi_mr = p_mr.ofi_mrs[i];
    uint64_t rkey = fi_mr_key((struct fid_mr*)(ofi_mr));
    std::memcpy(&ret[0] + i * sizeof(uint64_t), (void*)&rkey, sizeof(uint64_t));
  }
  return ret;
}
...
inline std::vector<net_status_t> ofi_net_device_impl_t::poll_comp(int max_polls)
{
  ...
  for (int i = 0; i < net_endpoints.size(); i++) {
    next_to_poll = (next_to_poll + 1) % net_endpoints.size();
    auto p_ofi_endpoint = static_cast<ofi_net_endpoint_impl_t*>(
        net_endpoints[next_to_poll].p_impl);
    // LCISI_OFI_CS_TRY_ENTER(endpoint_p, LCI_BACKEND_TRY_LOCK_POLL, 0)
    ssize_t ne = fi_cq_read(p_ofi_endpoint->ofi_cq, fi_entry.data(), max_polls);
    ...
    if (ne > 0) {
      ...
      ssize_t ret_cqerr = fi_cq_readerr(p_ofi_endpoint->ofi_cq, &error, 0);
