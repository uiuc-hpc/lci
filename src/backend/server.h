                                          ...
                                          LCIS_meta_t meta)
{
  ...
  if (ret == LCI_OK) {
    ...
    LCII_PCOUNTERS_WRAPPER(LCII_pcounters[LCIU_get_thread_id()].send_backend_failed_lock++);
    ret = LCI_ERR_RETRY;
  } else {
    LCII_PCOUNTERS_WRAPPER(LCII_pcounters[LCIU_get_thread_id()].send_backend_failed_nomem++);
    ret = LCI_ERR_RETRY;
  }
  return ret;
}
