    ...
    if item["category"] == "resource":
      for attr in item["attrs"]:
        if "default_value" in attr and attr["default_value"] is not None:
          type_name = attr["type"]
          env_var = f"LCIXX_ATTR_{attr['name'].upper()}"
          attr_name = attr['name']
          if "enum" in attr["trait"]:
...
{dict_decl}
    ...
    LCIXX_Log(LOG_INFO, "env", "set {attr_name} to be %d\\n",
              static_cast<int>(g_default_attr.{attr_name}));
  }}\n"""
          else:
            text += f"  g_default_attr.{attr_name} = get_env_or(\"{env_var}\", {attr['default_value']});\n"
            if type_name == "std::string":
              text += f"  LCIXX_Log(LOG_INFO, \"env\", \"set {attr_name} to be %s\\n\", g_default_attr.{attr_name}.c_str());\n"
            else:
              text += f"  LCIXX_Log(LOG_INFO, \"env\", \"set {attr_name} to be %d\\n\", static_cast<int>(g_default_attr.{attr_name}));\n"
