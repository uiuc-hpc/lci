...
resource_device := resource(
    ...
    [
        ...
        attr("bool", "shm_enable", default_value=1, comment="Whether to enable the internal intra-node shared-memory small-message transport."),
        attr("size_t", "shm_ring_size", default_value="64 * 1024", comment="Shared-memory inbound slot-array size in bytes."),
        attr("size_t", "shm_slot_size", default_value=128, comment="Shared-memory fixed slot size in bytes."),
        attr("size_t", "shm_max_message_size", default_value=0, comment="Maximum payload size eligible for shared memory; 0 derives it from the slot and packet size."),
        attr("size_t", "shm_max_cas_attempts", default_value=4, comment="Bounded CAS attempts for shared-memory producer/consumer progress."),
