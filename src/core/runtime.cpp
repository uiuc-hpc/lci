 ...
 *************************************************************/
int runtime_t::get_rank() const {
    return p_impl->get_rank();
}

int runtime_t::get_nranks() const {
...
void runtime_impl_t::initialize(runtime_t runtime_) {
    ...
    if (config.backend != option_backend_t::none) {
        net_context = alloc_net_context_x(runtime).set_backend(config.backend).call();
    }
