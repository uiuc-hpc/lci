...
static inline void LCM_aqueue_init(LCM_aqueue_t* queue, uint64_t capacity)
{
  LCM_Assert(sizeof(LCM_aqueue_entry_t) == LCI_CACHE_LINE, "Unexpected sizeof(LCM_aqueue_entry_t) %lu\n", sizeof(LCM_aqueue_entry_t));
  queue->container = LCIU_memalign(LCI_CACHE_LINE, (capacity + 1) * sizeof(LCM_aqueue_entry_t));
  queue->top = 0;
  queue->top2 = 0;
  queue->bot = 0;
  queue->bot2 = 0;
...
static inline void LCM_aqueue_push(LCM_aqueue_t* queue, void *val) {
  ...
  // write to the slot
//  __sync_synchronize();
  LCM_DBG_Assert(queue->container[current_top % queue->length].data == NULL, "wrote to a nonempty value!\n");
  queue->container[current_top % queue->length].data = val;
//  __sync_synchronize();
  // update top2 to tell the consumers they can safely read this slot.
  while (true) {
    uint64_t ret = __sync_val_compare_and_swap(&queue->top2, current_top, current_top + 1);
    if (ret == current_top) {
      // succeed!
      break;
    }
...
//  LCM_DBG_Assert(current_top2 > current_bot, "bot %lu is ahead of top2 %lu!\n", current_bot, current_top2);
  uint64_t ret = __sync_val_compare_and_swap(&queue->bot, current_bot, current_bot + 1);
  if (ret != current_bot) {
...
#endif
//  __sync_synchronize();
  // now that we got the value, we can update bot2 to tell the producers they can
  // safely write to this entry.
  while (true) {
    uint64_t ret = __sync_val_compare_and_swap(&queue->bot2, current_bot, current_bot + 1);
    if (ret == current_bot) {
      // succeed!
      break;
    }
