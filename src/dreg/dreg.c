  ...
  while (vma != &vma_list && vma->start <= DREG_END(r)) {
    ...
    if (vma->list == NULL) {
      ...
      do {
        ...
        if (vma->start == vma->prev->end + 1 && compare_lists(vma, vma->prev)) {
          t = vma;
          vma = vma->prev;
          vma->end = t->end;
          vma->next = t->next;
        ...
        if (vma->end + 1 == vma->next->start && compare_lists(vma, vma->next)) {
          t = vma->next;
          vma->end = t->end;
          vma->next = t->next;
          vma->next->prev = vma;
...
int dreg_init()
{
  ...
  delayed_buf_region =
      malloc((unsigned)(sizeof(struct iovec) * rdma_ndreg_entries));
  ...
  deregister_mr_array =
      (dreg_entry**)malloc(sizeof(dreg_entry*) * rdma_ndreg_entries);
...
int dreg_finalize()
{
   ...
   * in the free list
   */
  if (dreg_all_list != NULL) free(dreg_all_list);

#if !defined(DISABLE_PTMALLOC)

  if (delayed_buf_region != NULL) free(delayed_buf_region);
...
#if !defined(DISABLE_PTMALLOC)
int have_dereg() { return pthread_equal(th_id_of_dereg_lock, pthread_self()); }
...
int have_dreg() { return pthread_equal(th_id_of_lock, pthread_self()); }
...
dreg_entry* dreg_new_entry(void* server, void* buf, size_t len)
{
  ...
  for (; i < rdma_num_hcas; ++i) {
    d->server = server;
    d->memhandle[i] =
        get_dma_mem(d->server, (void*)pagebase_low_p, register_nbytes);
...
find_buf:
  ...
  for (i = 0; i < npages; i++) {
    ...
    end = ((unsigned long)(((char*)addr) + DREG_PAGESIZE - 1)) >> DREG_PAGEBITS;
