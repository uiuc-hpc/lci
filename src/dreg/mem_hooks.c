...
static void set_real_munmap_ptr()
{
  ...
  if (NULL != handle) {
    ...
    LCM_Assert(munmap_ptr != mvapich_munmap_ptr,
               ...
               "This error usually means that the program is linked with both "
               "static and shared MVAPICH2 libraries.\n"
...
{
  if (mem_hook_init && (size > 0) && !mvapich2_minfo.is_mem_hook_finalized) {
...
#if !(defined(HAVE_SYSCALL) && defined(__NR_munmap))
  if (!mvapich2_minfo.munmap && !resolving_munmap) {
    resolving_munmap = 1;
    set_real_munmap_ptr();
    resolving_munmap = 0;
    if (NULL != store_buf) {
...
#endif
  if (mem_hook_init && !mvapich2_minfo.is_mem_hook_finalized) {
...
void* mvapich2_sbrk(intptr_t delta)
{
  if (delta < 0) {
    ...
    mvapich2_mem_unhook((void*)((uintptr_t)current_brk + delta), -delta);
