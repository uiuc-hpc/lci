...
static inline void lc_init_req(void* buf, size_t size, LCI_Request* req)
{
  req->buffer = buf;
  req->size = size;
}

static inline void lc_prepare_rtr(LCI_Endpoint ep, void* src, size_t size, lc_packet* p)
{
...
static inline void lc_ce_glob(LCI_Endpoint ep)
{
...
static inline void lc_ce_signal(LCI_Endpoint ep, lc_packet* p)
{
  LCI_Request* req = p->context.req;
  dprintf("signal\n");
...
static inline void lc_ce_queue(LCI_Endpoint ep, lc_packet* p)
{
  LCI_Request* req = p->context.req;
  // req->parent = p;
  lc_cq_push(ep->cq, (void*) req);
}

static inline void lc_handle_rtr(LCI_Endpoint ep, lc_packet* p)
{
  dprintf("Recv RTR %p\n", p);
  lc_pk_init(ep, -1, LC_PROTO_LONG, p);

  lc_server_rma_rtr(ep->server, p->context.req->__reserved__,
      (void*) p->data.rts.src_addr,
      p->data.rtr.tgt_addr, p->data.rtr.rkey, p->data.rts.size,
      p->data.rtr.comm_id, p);
}

static inline void lc_handle_rts(LCI_Endpoint ep, lc_packet* p)
{
  dprintf("Recv RTS: %p\n", p);
...
static inline void lc_serve_recv_imm(LCI_Endpoint ep, lc_packet* p, lc_proto proto, const long cap)
{
  p->context.req->tag = PROTO_GET_META(proto);
  proto = PROTO_GET_PROTO(proto);
  p->context.req->buffer = &p->data;
  lc_ce_dispatch(ep, p, cap);
}

static inline void lc_serve_recv_dyn(LCI_Endpoint ep, lc_packet* p, lc_proto proto, const long cap)
{
  p->context.req->tag = PROTO_GET_META(proto);
  proto = PROTO_GET_PROTO(proto);

  if (proto == LC_PROTO_DATA) {
    void* buf = ep->alloc(p->context.req->size, p->context.req->usr_ctx);
    memcpy(buf, &p->data, p->context.req->size);
    p->context.req->buffer = buf;
    lc_ce_dispatch(ep, p, cap);
  } else if (proto == LC_PROTO_RTS) {
    void* buf = ep->alloc(p->data.rts.size, p->context.req->usr_ctx);
    lc_init_req(buf, p->data.rts.size, p->context.req);
    lc_handle_rts(ep, p);
...
static inline void lc_serve_recv_expl(LCI_Endpoint ep, lc_packet* p, lc_proto proto, const long cap)
{
  p->context.req->tag = PROTO_GET_META(proto);
  ...
  if (proto == LC_PROTO_DATA) {
    ...
    if (!lc_hash_insert(ep->tbl, key, &value, SERVER)) {
      LCI_Request* req = (LCI_Request*) value;
    ...
    lc_value value = (lc_value)p;
    if (!lc_hash_insert(ep->tbl, key, &value, SERVER)) {
      p->context.req = (LCI_Request*) value;
      lc_handle_rts(ep, p);
    }
  } else if (proto == LC_PROTO_RTR) {
    lc_handle_rtr(ep, p);
...
static inline void lc_serve_recv(lc_packet* p, lc_proto proto)
{
  ...
  LCI_Endpoint ep = lcg_endpoint[PROTO_GET_RGID(proto)];
