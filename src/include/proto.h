...
#define MAKE_PROTO(reid, proto, meta)  (proto | (reid << 2) | (meta << 16))

#define PROTO_GET_PROTO(proto) (proto         & 0b011)
#define PROTO_GET_REID(proto)  ((proto >> 2)  & 0b01111111)
...
{
  // p->context.req = req;
  uintptr_t rma_mem = lc_server_rma_reg(ep->dev->handle, src, size);
...
void lc_serve_recv_alloc(struct lci_ep* ep, lc_packet* p, uint32_t proto, func_cb complete)
{
  ...
  if (proto == LC_PROTO_DATA) {
      ...
      proto = MAKE_PROTO(p->data.rts.reid, LC_PROTO_RTR, 0);
      lc_server_send(ep->dev->handle, ep, p->context.req->rhandle, &p->data, sizeof(struct packet_rtr), p, proto);
  } else if (proto == LC_PROTO_RTR) {
      p->context.proto = MAKE_PROTO(p->data.rts.reid, LC_PROTO_LONG, 0);
      p->context.ep = ep;
      lc_server_rma_rtr(ep->dev->handle, p->context.req->rhandle,
...
LC_INLINE
void lc_serve_recv(lc_dev dev, lc_packet* p, uint32_t proto, const long server_cap)
{
