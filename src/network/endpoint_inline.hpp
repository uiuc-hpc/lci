                                          ...
                                          bool force_post)
{
  ...
  if (!force_post && !backlog_queue.is_empty()) {
    ...
    error = post_send_impl(rank, buffer, size, mr, imm_data, ctx);
  }
  if (error.is_retry()) {
    LCI_PCOUNTER_ADD(net_send_post_retry, 1);
    if (!allow_retry) {
                                         ...
                                         bool force_post)
{
  ...
  if (!force_post && !backlog_queue.is_empty()) {
    ...
    error = post_put_impl(rank, buffer, size, mr, base, offset, rkey, ctx);
  }
  if (error.is_retry()) {
    LCI_PCOUNTER_ADD(net_send_post_retry, 1);
    if (!allow_retry) {
                                         ...
                                         bool force_post)
{
  ...
  if (!force_post && !backlog_queue.is_empty()) {
    ...
    error = post_get_impl(rank, buffer, size, mr, base, offset, rkey, ctx);
  }
  if (error.is_retry()) {
    LCI_PCOUNTER_ADD(net_send_post_retry, 1);
    if (!allow_retry) {
