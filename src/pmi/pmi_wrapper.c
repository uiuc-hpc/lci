...
int lcm_pm_initialized() {
  return LCM_PM_ops.is_initialized();
}
int lcm_pm_get_rank() {
  return LCM_PM_ops.get_rank();
}
int lcm_pm_get_size() {
  return LCM_PM_ops.get_size();
}
void lcm_pm_publish(char *key, char *value) {
  LCM_PM_ops.publish(key, value);
}
void lcm_pm_getname(char *key, char *value) {
  LCM_PM_ops.getname(key, value);
}
void lcm_pm_barrier() {
  LCM_PM_ops.barrier();
}
