...
void qp_init(struct ibv_qp* qp, int port) {
...
void qp_to_rtr(struct ibv_qp* qp, int dev_port, struct ibv_port_attr* port_attr, struct conn_ctx* rmtctx_) {
  struct ibv_qp_attr attr;
  memset(&attr, 0, sizeof(struct ibv_qp_attr));
...
void setup_wr(struct ibv_send_wr* wr, uintptr_t id, struct ibv_sge* l,
...
void ibv_server_write_rma(ibv_server* s, int rank, void* from, void* to,
...
uint32_t ibv_server_heap_rkey(ibv_server* s, int node)
{
  return s->conn[node].rkey;
}

void* ibv_server_heap_ptr(ibv_server* s) { return s->heap_ptr; }
