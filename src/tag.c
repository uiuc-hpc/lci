...
lc_status lc_send_tag_p(lch* mv, struct lc_pkt* pkt, int rank, lc_info* info, lc_req* ctx)
{
  ...
  if (size <= (int) SHORT_MSG_SIZE) {
    lci_send(mv, src, p->context.size, rank, info->tag, LC_PROTO_DATA | LC_PROTO_TAG, p);
    ...
    lci_send(mv, &p->data, sizeof(struct packet_rts),
             rank, info->tag, LC_PROTO_RTS | LC_PROTO_TAG, p);
  }
  return LC_OK;
}

lc_status lc_send_tag(lch* mv, const void* src, size_t size, int rank, lc_info* info, lc_req* ctx)
{
  LC_POOL_GET_OR_RETN(mv->pkpool, p);
  if (size <= (int) SHORT_MSG_SIZE) {
    lci_send(mv, src, size, rank, info->tag, LC_PROTO_DATA | LC_PROTO_TAG, p);
    ctx->int_type = LC_REQ_DONE;
  } else {
    INIT_CTX(ctx);
    ctx->tag = info->tag;
...
lc_status lc_recv_tag(lch* mv, void* src, size_t size, int rank, lc_info* info, lc_req* ctx)
{
  INIT_CTX(ctx);
  ctx->tag = info->tag;
  ctx->lsync = info->rsync;
  lc_key key = lc_make_key(rank, info->tag);
  ...
  if (!lc_hash_insert(mv->tbl, key, &value, CLIENT)) {
    ...
    if (p->context.proto & LC_PROTO_DATA) {
      ...
      lci_send(mv, &p->data, sizeof(struct packet_rtr),
          rank, info->tag, LC_PROTO_RTR | LC_PROTO_TAG, p);
    }
  }
  return LC_OK;
}
