...
int iompi_coll_base_allreduce_intra_recursivedoubling(const void* sbuf,
                                                      void* rbuf, size_t count,
                                                      ompi_op_t op, lc_ep comm,
                                                      lc_colreq* req)
{
   ...
   *        - Even ranks less than 2 * extra_ranks send their data to (rank +
   * 1), and sets new rank to -1.
  ...
  if (rank < (2 * extra_ranks)) {
    if (0 == (rank % 2)) {
      lc_col_send(tmpsend, count, (rank + 1), MCA_COLL_BASE_TAG_IALLREDUCE,
                  comm, req);
      newrank = -1;
    } else {
      lc_col_recv(tmprecv, count, (rank - 1), MCA_COLL_BASE_TAG_IALLREDUCE,
                  comm, req);
  ...
  for (distance = 0x1; distance < adjsize; distance <<= 1) {
    ...
    /* Exchange the data */
    // lc_col_send(tmpsend, count, remote, MCA_COLL_BASE_TAG_IALLREDUCE, comm,
    // req); lc_col_recv(tmprecv, count, remote, MCA_COLL_BASE_TAG_IALLREDUCE,
    // comm, req);
    lc_col_sendrecv(tmpsend, tmprecv, count, remote,
   ...
   *        (rank - 1)
   *        - Even ranks less than 2 * extra_ranks receive result from (rank +
   * 1)
   *        */
  if (rank < (2 * extra_ranks)) {
    if (0 == (rank % 2)) {
      lc_col_recv(rbuf, count, (rank + 1), MCA_COLL_BASE_TAG_IALLREDUCE, comm,
                  req);
      tmpsend = (char*)rbuf;
    } else {
      lc_col_send(tmpsend, count, (rank - 1), MCA_COLL_BASE_TAG_IALLREDUCE,
